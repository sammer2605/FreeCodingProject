...
class MotionTimelapse:
    """Simple motion-controlled timelapse."""
    def __init__(self, num_shots, interval, move_mm_per_shot=10.0, slider_port="/dev/ttyACM0"):
        self.intervalometer = Intervalometer(num_shots, interval)
    ...
    def run(self, feed_rate=100):
        for i in range(self.num_shots):
            ...
            if i != self.num_shots - 1:
                print(f"Moving slider {self.move_mm_per_shot:.2f} mm at feed {feed_rate} mm/min")
...
if __name__ == "__main__":
    ...
    move_mm_per_shot = float(input("Enter distance to move per shot (mm): "))
    feed_rate = float(input("Enter feed rate (mm/min): "))

    timelapse = MotionTimelapse(num_shots, interval, move_mm_per_shot)
    try:
